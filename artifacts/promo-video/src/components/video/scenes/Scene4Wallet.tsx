...
export function Scene4Wallet() {
  useEffect(() => {
    setTimeout(() => window.playSfx?.(), 300);
    setTimeout(() => window.speakText?.("Withdraw to bKash or cryptocurrency"), 100);
  }, []);
  ...
  return (
    ...
    >
      <PhoneFrame src={`${import.meta.env.BASE_URL}screen-wallet.jpg`} alt="Wallet and Earnings" />
      ...
      >
        <div className="bg-[#0a0a1a]/90 backdrop-blur-md border border-[#00d4ff]/30 p-6 rounded-2xl text-center shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
          ...
          <p className="text-[#00d4ff] text-xl font-bold">To bKash & Crypto</p>
        </div>
      </motion.div>
    </motion.div>
  );
